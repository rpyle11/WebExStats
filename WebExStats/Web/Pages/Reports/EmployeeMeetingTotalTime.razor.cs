...
{
    public partial class EmployeeMeetingTotalTime
    {
        [CascadingParameter]
        ...
        private List<TotalEmployeeTimeInAllMeetingsDto> GridData { get; set; } = new();
        ...
        private async Task LoadGridData()
        {
            try
            {
                ...
                var data = await ApiDataService.GetEmployeeTotalTimeInMeetings(parameters, AppUser);

                if (data != null)
                {
                    GridData = data.OrderByDescending(o => o.MeetingStart).ThenBy(t => t.Title).ToList();
                }
